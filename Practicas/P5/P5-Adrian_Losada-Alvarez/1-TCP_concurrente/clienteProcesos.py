...
def manejar_cliente(ip, puerto):
    ...
    address = (ip, puerto)
    try:
        print(f'Conectando con el servidor desde el proceso {os.getpid()}...')
        socket_client.connect(address)
        print(f'Conexión al servidor establecida desde el proceso {os.getpid()}')

        # Recepción de 2 mensajes
        for _ in range(2):
            msg = socket_client.recv(1024)
            print(f'Soy el proceso hijo {os.getpid()} y he recibido el mensaje: {msg.decode()}')
...
def cliente(ip, puerto, num_clientes):
    for _ in range(num_clientes):
        ...
        if pid == 0:  # Este bloque se ejecuta en el proceso hijo
            manejar_cliente(ip, puerto)
            break
