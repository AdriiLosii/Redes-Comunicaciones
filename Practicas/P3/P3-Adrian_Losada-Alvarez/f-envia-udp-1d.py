...
def envia(puerto_prop, ip, puerto_dest):
	...
	try:
		...
		# Definimos la dirección destino y el mensaje y lo enviamos
		address_dest = (ip, puerto_dest)
		array = [1.2, 34.0, 643.2, 32.7, 54.9, 20.5, 0.9, 32.1, 102.2]
		packed_array = b''

		# Float
		# Empaquetamos el array en formato float
		for float in array:
			packed_array += struct.pack('f', float)

		# Double:
		# Empaquetamos el array en formato double
		"""for double in array:
			packed_array += struct.pack('d', double)"""

		# Enviamos el mensaje empaquetado
		bytes_sent = socket_send.sendto(packed_array, address_dest)
		print('Bytes enviados: {}'.format(bytes_sent))
...
def main():
	try:
		...
		if(args.puerto_prop and args.ip and args.puerto_dest):
			envia(args.puerto_prop, verificar_IP(args.ip), args.puerto_dest)
