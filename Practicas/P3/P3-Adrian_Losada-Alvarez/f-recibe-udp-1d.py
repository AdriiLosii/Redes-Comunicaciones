...
def recibe(puerto_prop):
    ...
    try:
        ...
        # Float
        num_datos = len(info[0])//4     # Ya que los floats se codifican en 4 bytes
        unpacked_info = struct.unpack(str(num_datos)+'f', info[0])

        # Double
        """num_datos = len(info[0])//8     # Ya que los double se codifican en 8 bytes
        unpacked_info = struct.unpack(str(num_datos)+'d', info[0])"""

        print('Se ha conectado el cliente con IP: {}; y puerto: {}'.format(info[1][0], info[1][1]))
        print('Mensaje recibido: {}'.format(unpacked_info))
